...
describe('RIASEC Assessment: All A-Starting Combinations with Google Sheets Logging', () => {
  ...
  A_COMBINATIONS.forEach(code => {
    it(`runs RIASEC flow for A-code: ${code}`, () => {
      const testName = `Test_${code}`;
      
      cy.log('╔═══════════════════════════════════════╗');
      cy.log(`║  TESTING: ${code}                        ║`);
      cy.log('╚═══════════════════════════════════════╝');
      
      return fillBasicInfo(testName, code)
        .then(() => {
          const answers = generateAnswers(code);
          return cy.wrap(null).then(() => answerQuestions(answers));
        })
        .then(() => {
          return handleSubmitAndVerify(code);
        })
        .then(() => {
