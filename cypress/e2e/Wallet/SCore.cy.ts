...
describe('RIASEC Assessment: All R-Starting Combinations with Google Sheets Logging', () => {
  const PAUSE = 600;
  const APP_URL = 'https://job-recommendation-878524709646.asia-southeast1.run.app/';

  // 🔑 GOOGLE SHEETS CONFIG
  const GOOGLE_SHEETS_WEB_APP_URL =
    'https://script.google.com/macros/s/AKfycbxPLUrcDt2B0fFhVb7XA03SOsddKBpIXcv4bhny0hvZqc-tM1Y88RJAwXDKaaMIaWJYYg/exec';

  // ✅ QUESTION MAP (same mapping used across tests)
  ...
  function logToGoogleSheets(data: any) {
    cy.log('📊 Logging to Google Sheets...', JSON.stringify(data));
    return cy.request({
      method: 'POST',
  ...
  // selectInterests returns selected interest names (for logging)
    ...
    cy.get('body').then($body => {
      for (let i = 0; i < numToSelect && i < shuffled.length; i++) {
        ...
        if ($body.find(id).length) {
          ...
          selectedIds.push(id);
          cy.get(`label[for="${id.substring(1)}"]`).invoke('text').then(text => {
            selectedNames.push(text.trim());
            cy.log(`✓ Selected interest: ${text.trim()}`);
          });
        }
      }
      cy.log(`📋 Selected ${selectedIds.length} interests`);
    });
  ...
  function fillBasicInfo(testName: string, targetCode: string) {
    const testData: any = { targetCode, testName, timestamp: new Date().toISOString() };
  ...
  function generateAnswers(targetCode: string): Record<number, string> {
    ...
    for (let q = 1; q <= 30; q++) {
      ...
      cy.log(`${marker} Q${String(q).padStart(2, '0')}: A=${optionA} B=${optionB} → ${answer}`);
    }
  ...
  // Test RAI first with Google Sheets logging
