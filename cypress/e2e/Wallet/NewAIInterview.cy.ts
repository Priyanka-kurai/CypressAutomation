  ...
  it('should complete interview setup and answer questions successfully', () => {
    ...
    cy.visit('https://frontend-v2-712919537046.asia-southeast1.run.app', {
      onBeforeLoad(win) {
        ...
        const mockStream = {
          getTracks: () => [
            { kind: 'video', label: 'Mock Video', stop: () => {}, enabled: true },
            { kind: 'audio', label: 'Mock Audio', stop: () => {}, enabled: true }
          ],
          getVideoTracks: () => [{ kind: 'video', label: 'Mock Video', stop: () => {}, enabled: true }],
          getAudioTracks: () => [{ kind: 'audio', label: 'Mock Audio', stop: () => {}, enabled: true }],
          addEventListener: () => {},
          removeEventListener: () => {},
          active: true,
          id: 'mock-stream-id'
        ...
        // Stub getUserMedia to return the mock stream
    ...
    // Dynamically select from available roles
    cy.get('select').eq(1)
      .should('not.be.disabled')
      .find('option')
      .then($options => {
        // Get available roles (skip first placeholder)
        ...
        cy.get('select').eq(1).select(availableRoles[randomIndex]);
      });
    ...
    // ================= STEP 4 - Wait for interview page =================
    cy.wait(3000);
