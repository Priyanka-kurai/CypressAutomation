// cypress/e2e/riasec_all_c_combinations.cy.ts
...
describe('RIASEC Assessment: All C-Starting Combinations with Google Sheets Logging', () => {
  ...
  const GOOGLE_SHEETS_WEB_APP_URL =
    'https://script.google.com/macros/s/AKfycbwP0ChaoSSfl6o7VdWMFSKW7z0bJPEZQh2_q3AR67Dxm8Ge6JoFXd3KUmoU5rCqrMsT/exec';
  ...
  // Select interests and return selected names for logging
  ...
  function generateAnswers(targetCode: string): Record<number, string> {
    ...
    for (let q = 1; q <= 30; q++) {
      ...
      cy.log(`${marker} Q${q.toString().padStart(2, '0')}: A=${optionA} B=${optionB} → ${answer}`);
    }
  ...
  // Test CAR first with Google Sheets logging
  it('should produce CAR result and log to Google Sheets', () => {
    const targetCode = 'CAR';
