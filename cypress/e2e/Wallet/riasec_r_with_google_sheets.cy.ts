// cypress/e2e/riasec_with_google_sheets.cy.ts

describe('RIASEC Assessment: R Combinations with Google Sheets Logging', () => {
  const PAUSE = 600;
  ...
  const GOOGLE_SHEETS_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbxPLUrcDt2B0fFhVb7XA03SOsddKBpIXcv4bhny0hvZqc-tM1Y88RJAwXDKaaMIaWJYYg/exec';
  
  const QUESTION_MAP = {
    1: { A: 'R', B: 'I' }, 2: { A: 'I', B: 'R' }, 3: { A: 'R', B: 'A' },
    4: { A: 'A', B: 'R' }, 5: { A: 'R', B: 'S' }, 6: { A: 'S', B: 'R' },
    7: { A: 'R', B: 'E' }, 8: { A: 'E', B: 'R' }, 9: { A: 'R', B: 'C' },
    10: { A: 'C', B: 'R' }, 11: { A: 'I', B: 'A' }, 12: { A: 'A', B: 'I' },
    13: { A: 'I', B: 'S' }, 14: { A: 'S', B: 'I' }, 15: { A: 'I', B: 'E' },
    16: { A: 'E', B: 'I' }, 17: { A: 'I', B: 'C' }, 18: { A: 'C', B: 'I' },
    19: { A: 'A', B: 'S' }, 20: { A: 'S', B: 'A' }, 21: { A: 'A', B: 'E' },
    22: { A: 'E', B: 'A' }, 23: { A: 'A', B: 'C' }, 24: { A: 'C', B: 'A' },
    25: { A: 'S', B: 'E' }, 26: { A: 'E', B: 'S' }, 27: { A: 'S', B: 'C' },
    28: { A: 'C', B: 'S' }, 29: { A: 'E', B: 'C' }, 30: { A: 'C', B: 'E' }
  };
  ...
  function selectInterestsForCode(targetCode: string) {
    ...
    // Return selected interests for logging
  ...
  function handleSubmitAndVerify(targetCode: string) {
    ...
    return cy.get('body').then($body => {
      ...
      // Extract top 3 job recommendations
      cy.get('body').then($recommendBody => {
        const top3Jobs: string[] = [];
        ...
        if ($jobCards.length > 0) {
          ...
          $jobCards.slice(0, 15).each((index, card) => {
            ...
            if (jobTitle && matchPercent) {
              const jobEntry = `${index + 1}. ${jobTitle} (${matchPercent})`;
              top3Jobs.push(jobEntry);
              cy.log(`  📌 Job ${index + 1}: ${jobTitle} - ${matchPercent}`);
            }
          });
          
          testData.top3Jobs = top3Jobs.join(' | ');
          cy.log(`📋 Top 15 Jobs: ${testData.top15Jobs}`);
        } else {
  ...
  // Test RAI
  it('should produce RAI result and log to Google Sheets', () => {
